...
# --- Función y gradiente de f(x,y) = x³ - y³ ---
z_fine = x_fine**3 - y_fine**3
dfdx = 3 * x**2
dfdy = -3 * y**2
...
fig.colorbar(contf, ax=ax, label='f(x,y) = x³ − y³')

# Campo vectorial (normalizado, coloreado por magnitud)
...
ax.set_title('Campo vectorial $\\nabla f = (3x^2,\\ -3y^2)$ sobre el plano x-y')
ax.set_xlabel('x')
ax.set_ylabel('y')
